...
export async function saveStudentScore(
  ...
  try {
    console.log("[v0] saveStudentScore called with:", {
      studentId,
      subjectId,
      classId,
      sessionId,
    ...
    console.log("[v0] Calculated values:", { total, grade, remark })
    ...
    for (const config of assessmentConfigs) {
      console.log("[v0] Processing assessment:", config.name)
      ...
      if (typeQueryError) {
        console.error("[v0] Error querying assessment type:", typeQueryError)
        return { success: false, error: `Failed to query assessment type: ${typeQueryError.message}` }
      }

      if (existingType) {
        assessmentTypeId = existingType.id
        console.log("[v0] Found existing assessment type:", assessmentTypeId)
      } else {
        // Create assessment type if it doesn't exist
        console.log("[v0] Creating new assessment type:", config.name)
        ...
        if (typeError) {
          console.error("[v0] Error creating assessment type:", typeError)
          return { success: false, error: `Failed to create assessment type: ${typeError.message}` }
        }
        assessmentTypeId = newType.id
        console.log("[v0] Created assessment type:", assessmentTypeId)
      }
      ...
      if (assessmentQueryError) {
        console.error("[v0] Error querying assessment:", assessmentQueryError)
      ...
      if (existingAssessment) {
        ...
        console.log("[v0] Found existing assessment:", assessmentId)
      } else {
        ...
        console.log("[v0] Created assessment:", assessmentId)
      }

      // Upsert student score
      console.log("[v0] Upserting student score:", {
        student_id: studentId,
      ...
      if (scoreError) {
        console.error("[v0] Error saving score:", scoreError)
        return { success: false, error: `Failed to save score: ${scoreError.message}` }
      }
      console.log("[v0] Successfully saved score for", config.name)
    }

    // Update or create student result summary
    console.log("[v0] Updating student result summary")
    await updateStudentResult(studentId, classId, sessionId, termId, supabase)

    console.log("[v0] saveStudentScore completed successfully")
    return { success: true, total, grade, remark }
  } catch (error) {
    console.error("[v0] Error in saveStudentScore:", error)
...
async function updateStudentResult(
  ...
  assessments.forEach(assessment => {
    const subjectScores_arr = scores
  ...
  const passMarkMap = new Map(
    (classSubjects || []).map(cs => [cs.subject_id, cs.pass_mark])
  )
